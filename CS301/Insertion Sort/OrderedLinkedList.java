...
public class OrderedLinkedList<T> extends LinkedListClass<T>
{
	...
	{
		LinkedListNode<T> pivot = this.first.link;
		LinkedListNode<T> trailing = this.first;
		T temp = null;
		
		while(pivot.link != null)
		{
			
			while (trailing.link != pivot.link)
			{
				Comparable<T> test = (Comparable<T>)pivot.info;
				if(test.compareTo((T)trailing.info) < 0)
				{
					temp = (T)pivot.info;
					pivot.info = (T)trailing.info;
					trailing.info = (T)temp;
